...
st.write(
    ...
    "then compared with **Gemini**, and a PDF report will be generated."
)

with st.sidebar:
    st.header("Settings")
    api_status = "API key loaded" if GEMINI_API_KEY else "No API key found"
...
if run_button:
    if not uploaded_files:
        ...
        st.error("Gemini API key is missing. Set it in your .env file.")
    else:
        pdf_contents = {}
        ...
        st.info("Running Gemini comparison...")
        ...
        with st.spinner("Comparing documents with Gemini..."):
