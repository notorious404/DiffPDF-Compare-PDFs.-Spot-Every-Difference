...
function initFormSubmit() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                latestReportUrl = result.report_url || null;
            ...
            clearInterval(statusInterval);
            loadingOverlay.classList.add('hidden');
            alert("Network error. Please make sure the Flask server is running and try again.");
            console.error(error);
        }
    });
...
function initControls() {
    ...
    document.getElementById('btn-download-report').addEventListener('click', () => {
        if (!latestReportUrl) {
            alert("Please compare two PDFs first, then download the report.");
            return;
        }
